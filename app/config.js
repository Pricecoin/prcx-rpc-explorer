...
let baseUrl = (process.env.PRCXEXP_BASEURL || "/").trim();
...
let cdnBaseUrl = (process.env.PRCXEXP_CDN_BASE_URL || ".").trim();
while (cdnBaseUrl.endsWith("/")) {
	cdnBaseUrl = cdnBaseUrl.substring(0, cdnBaseUrl.length - 1);
}

let s3BucketPath = (process.env.PRCXEXP_S3_BUCKET_PATH || "").trim();
...
const currentCoin = process.env.PRCXEXP_COIN || "PRCX";

const rpcCred = credentials.rpc;


const cookieSecret = process.env.PRCXEXP_COOKIE_SECRET
 || (rpcCred.password && crypto.createHmac('sha256', JSON.stringify(rpcCred))
                               .update('prcx-rpc-explorer-cookie-secret').digest('hex'))
 || "0x000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f";


const electrumServerUriStrings = (process.env.PRCXEXP_ELECTRUM_SERVERS || process.env.PRCXEXP_ELECTRUMX_SERVERS || "").split(',').filter(Boolean);
...
[
	"PRCXEXP_DEMO",
	"PRCXEXP_PRIVACY_MODE",
	"PRCXEXP_NO_INMEMORY_RPC_CACHE",
	"PRCXEXP_RPC_ALLOWALL",
	"PRCXEXP_ELECTRUM_TXINDEX",
	"PRCXEXP_UI_HIDE_INFO_NOTES",
...
const slowDeviceMode = (process.env.PRCXEXP_SLOW_DEVICE_MODE.toLowerCase() == "true");

module.exports = {
	host: process.env.PRCXEXP_HOST || "127.0.0.1",
	port: process.env.PORT || process.env.PRCXEXP_PORT || 3002,
	secureSite: process.env.PRCXEXP_SECURE_SITE == "true",
	...
	displayDefaults: {
		displayCurrency: (process.env.PRCXEXP_DISPLAY_CURRENCY || "prcx"),
		localCurrency: (process.env.PRCXEXP_LOCAL_CURRENCY || "usd"),
		theme: (process.env.PRCXEXP_UI_THEME || "dark"),
		timezone: (process.env.PRCXEXP_UI_TIMEZONE || "local")
	},

	cookieSecret: cookieSecret,

	privacyMode: (process.env.PRCXEXP_PRIVACY_MODE.toLowerCase() == "true"),
	slowDeviceMode: slowDeviceMode,
	demoSite: (process.env.PRCXEXP_DEMO.toLowerCase() == "true"),
	queryExchangeRates: (process.env.PRCXEXP_NO_RATES.toLowerCase() != "true" && process.env.PRCXEXP_PRIVACY_MODE.toLowerCase() != "true"),
	noInmemoryRpcCache: (process.env.PRCXEXP_NO_INMEMORY_RPC_CACHE.toLowerCase() == "true"),
	
	rpcConcurrency: (process.env.PRCXEXP_RPC_CONCURRENCY || (slowDeviceMode ? 3 : 10)),

	filesystemCacheDir: (process.env.PRCXEXP_FILESYSTEM_CACHE_DIR || path.join(process.cwd(),"./cache")),

	noTxIndexSearchDepth: (+process.env.PRCXEXP_NOTXINDEX_SEARCH_DEPTH || 3),

	cdn: {
		active: (cdnBaseUrl == "." ? false : true),
		s3Bucket: process.env.PRCXEXP_S3_BUCKET,
		s3BucketRegion: process.env.PRCXEXP_S3_BUCKET_REGION,
		s3BucketPath: s3BucketPath,
		baseUrl: cdnBaseUrl
	},

	rpcBlacklist:
		process.env.PRCXEXP_RPC_ALLOWALL.toLowerCase() == "true"  ? []
		: process.env.PRCXEXP_RPC_BLACKLIST ? process.env.PRCXEXP_RPC_BLACKLIST.split(',').filter(Boolean)
		: [
		"addnode",
		"backupwallet",
		"bumpfee",
	...
	addressApi: process.env.PRCXEXP_ADDRESS_API,
	electrumTxIndex: process.env.PRCXEXP_ELECTRUM_TXINDEX != "false",
	electrumServers: electrumServers,

	redisUrl:process.env.PRCXEXP_REDIS_URL,

	site: {
		hideInfoNotes: process.env.PRCXEXP_UI_HIDE_INFO_NOTES,
		homepage:{
			recentBlocksCount: parseInt(process.env.PRCXEXP_UI_HOME_PAGE_LATEST_BLOCKS_COUNT || (slowDeviceMode ? 5 : 10))
		},
		blockTxPageSize: (slowDeviceMode ? 10 : 20),
		addressTxPageSize: 10,
		txMaxInput: (slowDeviceMode ? 3 : 15),
		browseBlocksPageSize: parseInt(process.env.PRCXEXP_UI_BLOCKS_PAGE_BLOCK_COUNT || (slowDeviceMode ? 10 : 25)),
	...
	/* 13 */	{name:"Whitepaper Extractor", url:"./pricecoinx-whitepaper", desc:"Extract the PricecoinX whitepaper from data embedded within the blockchain.", iconClass:"bi-file-earmark-text"},
	...
	/* 17 */	{name:"Quotes", url:"./quotes", desc:"Curated list of PricecoinX-related quotes.", iconClass:"bi-chat-quote"},

	/* 18 */	{name:"UTXO Set", url:"./utxo-set", desc:"View the latest UTXO Set.", iconClass:"bi-list-columns"},

	/* 19 */	{name:"Holidays", url:"./holidays", desc:"Curated list of PricecoinX 'Holidays'.", iconClass:"bi-calendar-heart"},
